...
if os.path.isdir('images/' + name):  #使用姓名做為資料夾名稱
    ...
    while index > 0:  #取樣
        ret, frame = cap.read()  # ret 為布林值的變數， frame 為影像變數
        frame = cv2.flip(frame, 1)  # 拍照是左右相反的，要調整回來
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # 
        faces = face_cascade.detectMultiScale(gray, 1.1, 3) # 偵測人臉 # 1.1為偵測比例 # 3 為誤檢率參數
        for (x, y, w, h) in faces:
            frame = cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 3)  #框選臉部
            image = cv2.resize(gray[y: y + h, x: x + w], (400, 400))  #存檔圖片大小 # 固定大小
            saveImg(image, index)
            sleep(0.1)
            index += 1
            if index > total:
